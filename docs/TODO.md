...
### `AGENTS.md` references `tsconfig.app.json` which does not exist

`AGENTS.md` instructs agents to `Read tsconfig.app.json to understand what path shortcuts exists`. This file is a Vite convention — the project uses Next.js and only has `tsconfig.json`. The instruction should reference `tsconfig.json` instead. Cannot edit `AGENTS.md` (it says "DO NOT EDIT THIS FILE") — flag this to the repo owner.
